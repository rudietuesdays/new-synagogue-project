...
function maisha_body_classes( $classes ) {
	...
	// Add class for the header layouts.
	if ( 'fixed-mobile-menu' === get_theme_mod( 'maisha_mobile_menu_layout' ) ) {
		$classes[] = 'fixed-mobile-menu';
	}
	if ( 'default-mobile-menu' === get_theme_mod( 'maisha_mobile_menu_layout' ) ) {
		$classes[] = 'default-mobile-menu';
	}
	
	// Adds a class of no-sidebar to sites without active blog sidebar.
	if ( ! is_active_sidebar( 'sidebar-1' ) ) {
		$classes[] = 'no-sidebar';
	}
	
	// Adds a class of no-sidebar to sites without active default sidebar.
	if ( ! is_active_sidebar( 'sidebar-5' ) ) {
		$classes[] = 'no-default-sidebar';
	}
